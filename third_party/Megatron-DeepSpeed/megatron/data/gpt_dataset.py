...
def build_train_valid_test_datasets(args, data_prefix, data_impl, splits_string,
    ...
    if data_prefix:
        print("Single data path provided for train, valid & test")

        # Single dataset.
        if len(data_prefix) == 1:
            return _build_train_valid_test_datasets(args, data_prefix[0],
        ...
        for i in range(len(prefixes)):
            train_ds, valid_ds, test_ds = _build_train_valid_test_datasets(
                args, prefixes[i], data_impl, splits_string,
...
def _build_train_valid_test_datasets(args, data_prefix, data_impl, splits_string,
                                     train_valid_test_num_samples,
                                     seq_length, seed, skip_warmup,
                                     return_doc_ids=False, *,
                                     data_cache_path=None):
    ...
    def build_dataset(index, name):
        ...
        if splits[index + 1] > splits[index]:
            ...
            dataset = GPTDataset(args, name, data_prefix, documents, indexed_dataset,
                                 splits_string,
...
def build_dataset(args, dataset_name, data_prefix, data_impl,
    ...
    dataset = None
    if len(data_prefix) == 1:
        dataset = _build_dataset(args, dataset_name, data_prefix[0], data_impl,
        ...
        datasets = []
        for i in range(len(prefixes)):
            ds = _build_dataset(args, dataset_name, prefixes[i], data_impl,
...
def _build_dataset(args, dataset_name, data_prefix, data_impl, splits_string,
    ...
    dataset = GPTDataset(args, dataset_name, data_prefix, documents, indexed_dataset,
...
class GPTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, name, data_prefix, documents, indexed_dataset,
                 splits_string, num_samples, seq_length, seed,
                 return_doc_ids=False, *,
                 data_cache_path=None):
        ...
        self.doc_idx, self.sample_idx, self.shuffle_idx, self.desc, self.desc_hash = \
            _build_index_mappings(self.args, self.name, data_prefix,
...
def _build_index_mappings(args, name, data_prefix, documents, sizes,
                          splits_string, num_samples, seq_length, seed,
                          *,
                          data_cache_path):
    ...
    if build_indices and (device_id == 0):
