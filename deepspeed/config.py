...
class DeepSpeedConfig(object):
    ...
    def _batch_assertion(self):
        ...
        # NOTE: handle this with care, as it will not detect cases that the user makes a mistake on the given DP
        if (train_batch != micro_batch * grad_acc * self.world_size):
            logger.warning("The global batch size is not perfectly divisible by (dp_size * mbs). This might be on-purpose (set by sailor) or by user mistake")
            rem_ubatches = train_batch - (micro_batch * grad_acc * self.world_size)
            if (self.dp_rank < rem_ubatches):
