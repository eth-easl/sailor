...
class DeepSpeedEngine(Module):
    ...
    def __init__(self,
        ...
        self.dp_groups_tps = [] # holds the TMP degrees of all
    ...
    def _broadcast_model(self):
        ...
        for dp_group in dp_group_list:
            ...
            all_tps = [x.item() for x in tps_of_ranks]
            self.dp_groups_tps.append(all_tps)
            ...
            for n, p in self.module.named_parameters():
                if torch.is_tensor(p) and is_replicated(p):
                    ...
                    if dim_param==-1:
                        ...
                        param_size = p.shape[dim_param]
                        offset = int(param_size * self.mp_world_size/bc_src_tp)
                        dist.broadcast(p.data[idx*offset:(idx+1)*offset], bc_src, group=dp_group)
                    elif dim_param==1:
                        param_size = p.shape[dim_param]
                        offset = int(param_size * self.mp_world_size/bc_src_tp)
                        if self.global_rank != bc_src:
                            # torch.broadcast needs contiguous tensors, so we allocate an temporary tensor to do the job.
                            # TODO: see how this can be optimized
                            temp_cont_tensor = torch.zeros((p.shape[0],offset),dtype=p.dtype).to(self.device)
                            dist.broadcast(temp_cont_tensor, bc_src, group=dp_group)
                            p.data[:,idx*offset:(idx+1)*offset].copy_(temp_cont_tensor)
                        else:
                            dist.broadcast(p.data, bc_src, group=dp_group)
                    else:
                        # only up to 2D tensors are supported, TODO: extend
                        raise NotImplementedError
                else:
                    print("DONT BROADCAST!")
            idx += 1
    ...
    def _get_gradients_for_reduction(self):
        ...
        for param_name, param in self.module.named_parameters():
            ...
            if is_moe_param(param):
                ...
                non_expert_grads[param_name] = grad_data

        return non_expert_grads, expert_grads

    def _reduce_non_expert_gradients(self, grads, elements_per_buffer, idx):
        split_sparse_tensor_buckets, split_dense_tensor_buckets = split_half_float_double_sparse(grads)
        if self.pipeline_parallelism:
            dp_group = self.mpu.get_data_parallel_group()[idx]
    ...
    def buffered_allreduce_fallback(self, grads=None, elements_per_buffer=500000000):
        if grads is None:
            if hasattr(self.optimizer, "get_grads_for_reduction"):
                ...
                non_expert_grads_dict, expert_grads = self._get_gradients_for_reduction()
                non_expert_grads = [value for _,value in non_expert_grads_dict.items()]
        else:
            assert not self.has_moe_layers, "attempting to reduce grads in unsupported way w.r.t. MoE"
            non_expert_grads = grads

        # for key,grad in non_expert_grads_dict.items():
        #     print(f"RANK {torch.distributed.get_rank()}, {key}, {grad.shape}")

        my_dp_groups = self.mpu.get_data_parallel_group()
        #print(f"RANK {torch.distributed.get_rank()}, my my_dp_groups is {my_dp_groups}")

        for idx,group in enumerate(my_dp_groups):
            ...
            for param_name, whole_grad in non_expert_grads_dict.items():
                ...
                if dim_param==-1:
                    ...
                    param_size = whole_grad.shape[dim_param]
                    offset = int(param_size * self.mp_world_size/max_tp) # TODO
                    grad = whole_grad[idx*offset:(idx+1)*offset]
                    non_expert_grads_group.append(grad)
                elif dim_param==1:
                    param_size = whole_grad.shape[dim_param]
                    offset = int(param_size * self.mp_world_size/max_tp) # TODO
                    grad = whole_grad[:,idx*offset:(idx+1)*offset]
                    non_expert_grads_group.append(grad)
            self._reduce_non_expert_gradients(non_expert_grads_group, elements_per_buffer, idx)
            # if self.has_moe_layers:
    ...
    def offload_states(self,
        ...
        assert param_offload_config is None or paeram_offload_config.device == OffloadDeviceEnum.none, "Moving states across devices is not supported for offloaded parameters."
