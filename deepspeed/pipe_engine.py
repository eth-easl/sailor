...
from ..utils import PartitionedTensor, reconstruct_tensor
...
def get_total_used_mem(device_id):
    ...
    return memory_used_values[device_id] * 1024 * 1024
...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        self.my_global_rank = torch.distributed.get_rank()
        ...
        mp_group = self.grid.get_model_parallel_group()
        dist.all_reduce(params_tensor, group=mp_group)
        ...
        self.my_pp_groups = p2p.get_pp_groups()
        # wait till all done
    ...
    def train_batch(self, data_iter=None, galvatron_profiler=None, iteration=0):
        ...
        Args:
            ...
            galvatron_profiler (optional): If galvatron is used as baseline, the profiler needs to be passed here
            iter (optional): If galvatron is used as baseline, the iteration counter needs to be passed here
        ...
        self.galvatron_profiler = galvatron_profiler
        self.iter = iteration
        ...
        if self.is_last_stage():
            ...
            if self.is_data_parallel:
                if self.agg_additional_losses is None:
                    for dp_group in self.mpu.get_data_parallel_group():
                        dist.all_reduce(agg_loss, group=dp_group)
                        agg_loss_dp = agg_loss / self.dp_world_size
                    agg_loss = agg_loss_dp
                else:
            ...
            assert self.global_rank in self.grid.pp_group_list[0] # TODO: fix for PP>1
            losses = [self.dp_group_loss, agg_loss]
            if self.agg_additional_losses is not None:
                losses += list(self.agg_additional_losses.values())
            losses = torch.stack(losses).float()
            if self.is_pipe_parallel:
                for pp_group in self.grid.pp_proc_group_list:
                    dist.broadcast(tensor=losses, src=self.global_rank, group=pp_group)
        else:
            # Get loss from last stage
            # src_rank = self.grid.stage_to_global(self.num_stages - 1)
            # assert src_rank in self.grid.pp_group_list[0] # TODO: fix for PP>1
            ...
            for pp_group in self.grid.pp_proc_group_list:
                all_pp_ranks = torch.distributed.get_process_group_ranks(pp_group)
                src_rank = all_pp_ranks[-1]
                dist.broadcast(tensor=losses, src=src_rank, group=pp_group)
    ...
    def _exec_forward_pass(self, buffer_id):
        ...
        if self.galvatron_profiler:
            self.galvatron_profiler.profile_memory(self.iter, "Before Forward")
        ...
        if self.galvatron_profiler:
            self.galvatron_profiler.profile_memory(self.iter, "After Forward")
    ...
    def _exec_backward_pass(self, buffer_id):
        ...
        if self.is_last_stage():
            ...
            if self.galvatron_profiler:
                self.galvatron_profiler.profile_memory(self.iter, "After Backward")
            return

        outputs = self.pipe_buffers['outputs'][buffer_id]
        #print(f"EARLY OUTPUTS IS {outputs}")
    ...
    def _exec_send_activations(self, buffer_id):
        ...
        if isinstance(outputs, torch.Tensor):
            ...
            for gidx in range(len(self.my_pp_groups)):
                for idx, buffer in enumerate(outputs):
                    p2p.send_with_group_idx(buffer, self.next_stage, gidx)
        else:
    ...
    def _exec_send_grads(self, buffer_id):
        ...
        if self.dynamic_shape or self.first_gradient_send:
        ...
        if isinstance(inputs, torch.Tensor):
            ...
            for gidx in range(len(self.my_pp_groups)):
                p2p.send_with_group_idx(inputs[0], self.prev_stage, gidx)
                p2p.send_with_group_idx(inputs[1], self.prev_stage, gidx)
    ...
    def _exec_recv_activations(self, buffer_id):
        ...
        self.pipe_buffers['inputs'][buffer_id] = recvd_list
    ...
    def _exec_recv_grads(self, buffer_id):
        ...
        if self.dynamic_shape or self.grad_layer_list is None:
            if isinstance(outputs, torch.Tensor):
                grad_layer = self._allocate_or_extend_buffers(0, list(outputs.size()), outputs.dtype)
            else:
                ...
                # ]
                # print(f"grad_layer is {grad_layer}")
                grad_layer = self._recv_tensor_meta(self.next_stage)
                self.grad_layer_list = [grad_layer for _ in range(len(self.my_pp_groups))]

        assert isinstance(outputs, tuple)
        for gidx in range(len(self.my_pp_groups)):
            for idx, buffer in enumerate(self.grad_layer_list[gidx]):
                # XXX GPT-2 hack
                ...
                p2p.recv_with_group_idx(buffer, self.next_stage, gidx)
    ...
    def _exec_optimizer_step(self, lr_kwargs=None):
        ...
        if self.galvatron_profiler:
            self.galvatron_profiler.profile_memory(self.iter, "After optimizer_step")
    ...
    def _exec_schedule(self, pipe_schedule):
        ...
        for step_cmds in pipe_schedule:
            ...
            for cmd in step_cmds:
                ...
                if self.print_memory:
                    total_mb = get_total_used_mem(device_id=self.device_id)
